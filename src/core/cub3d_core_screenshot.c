...
// TODO: Switch fprintf calls to proper error dealing functions
 ...
 * Set the bmp header.
 */
static void set_bmp_header(st_img *f, st_bmp_h *header)
{
    header->type = 0x4D42;
 ...
 */
static void write_header(int fd, st_bmp_h header)
{
 ...
 * Draw the screenshot.
 */
static void draw_bmp(int fd, st_img *f)
{
    ...
    while (l >= 0)
    {
        ...
        {
            fprintf(stderr, "\e[0;31mFailed to write pixels to image\e[0m\n");
...
void screenshot(st_img *f)
{
    ...
    if (!fd)
    {
        fprintf(stderr, "\e[0;31mCould not save the screen shoot\e[0m\n");
